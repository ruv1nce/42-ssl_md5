...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/05/02 23:58:00 by dfonarev          #+#    #+#             */
/*   Updated: 2019/05/02 23:58:01 by dfonarev         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			pf_validator(char *format, char *tmp, int spec_found)
{
	format--;
	while (*(++format))
	{
		if (*format == '%')
		{
			...
			while (*(++tmp))
			{
				...
				else if (pf_elf(*tmp) == -10 && !(NUM(*tmp)) && *tmp != '*'
						&& *tmp != '.')
					return (0);
			}
			if (!spec_found)
